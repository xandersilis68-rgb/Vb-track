...
## Overview
This project is a sophisticated web application for real-time body tracking, utilizing the latest MediaPipe Tasks API to detect and visualize human body, face, and hand landmarks for multiple people. It is designed to run on GitHub Pages, providing a responsive and interactive UI overlay to display tracked points and meshes with high precision. The application supports multiple model configurations, dynamic landmark and mesh rendering, and advanced features like interpolation for enhanced detection accuracy. It also supports uploading images or videos to overlay tracked landmarks and meshes, with optimized performance, multi-person tracking, and improved button functionality.
...
- **Interactive UI Overlay**: Displays tracking status, FPS, detected points count, and toggleable options for pose, face, hand landmarks, meshes, and multi-person mode. Includes error messages for debugging.
...
- **Button Reliability**: Enhanced event listeners with logging and error handling to ensure buttons ("Start Tracking," "Stop Tracking," "Process Upload") work consistently.
...
- **script.js**: Handles the core logic, including MediaPipe initialization, landmark and mesh processing, rendering, user interactions, media upload processing, and robust button event handling.
...
2. **Controls**:
   - **Start Tracking**: Click the "Start Tracking" button or press `s` to begin webcam-based tracking.
   - **Stop Tracking**: Click the "Stop Tracking" button or press `t` to halt processing.
...
3. **Monitoring**:
   ...
   - Status messages indicate tracking state, processing status, or errors (e.g., "Camera access denied" or "Unsupported file type").
...
- **Error Handling**: Includes try-catch blocks and UI feedback for robust processing and user feedback on failures (e.g., camera access errors, file processing issues).
- **Keyboard Shortcuts**: Enhances accessibility with key bindings for common actions, including mesh toggle (`m`).
- **Button Reliability**: Added console logging and error handling to ensure button functionality, with checks for initialization and state.
...
- **Performance**: Multi-person tracking, meshes, or heavy models may be resource-intensive on low-end devices. Use Lite mode and disable multi/meshes for better performance. Video processing may be slow for high-resolution or long videos.
...
## Troubleshooting
- **Buttons Not Working**:
   - Open the browser console (F12 -> Console) to check for errors when clicking buttons.
   - Verify button IDs match between `index.html` and `script.js`.
   - Ensure the MediaPipe scripts load correctly (check Network tab).
   - Test in a different browser or incognito mode to rule out extension conflicts.
   - Check console logs for button click events (added in `script.js`).
- **Camera Not Working**: Ensure camera permissions are granted and no other application is using the camera. Check the status message for errors like "Camera access denied."
