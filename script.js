...
const modelConfigs = {
    ...
    upper: {
        ...
        fallbackPoseModel: '/models/pose_landmarker_full.task',
    ...
    lite: {
        ...
        fallbackPoseModel: '/models/pose_landmarker_lite.task',
...
async function loadModelWithFallback(modelUrl, fallbackUrl) {
    try {
        ...
        if (response.ok) {
            console.log(`Model at ${modelUrl} is accessible`);
            return modelUrl;
        } else {
            console.warn(`Model at ${modelUrl} not accessible, trying fallback`);
            return fallbackUrl;
        }
    } catch (error) {
        console.warn(`Error accessing ${modelUrl}: ${error.message}, trying fallback`);
        return fallbackUrl;
    }
}
...
async function createLandmarkers() {
    ...
    try {
        ...
        console.log('Vision tasks resolver loaded');
        ...
        const poseModelUrl = await loadModelWithFallback(config.poseModel, config.fallbackPoseModel);
        ...
        const faceModelUrl = await loadModelWithFallback(faceModel, fallbackFaceModel);
        ...
        const handModelUrl = await loadModelWithFallback(handModel, fallbackHandModel);
        ...
        statusElement.textContent = 'Models initialized. Ready to start tracking.';
    } catch (error) {
        console.error('Error initializing landmarkers:', error);
        statusElement.textContent = `Failed to initialize models: ${error.message}. Check console for details.`;
...
async function processFrame() {
    ...
    inputCtx.save();
    if (isFlip) {
        inputCtx.translate(inputImage.width, 0);
        inputCtx.scale(-1, 1);
    }
    inputCtx.drawImage(sourceElement, 0, 0, inputImage.width, inputImage.height);
    inputCtx.restore();
    ...
    try {
        ...
        statusElement.textContent = 'Error processing frame. Check console for details.';
        stopProcessing();
        return;
    }
...
async function startTracking() {
    ...
    try {
        ...
        sourceElement.addEventListener('loadeddata', () => {
            console.log('Webcam loaded, starting frame processing');
        ...
        statusElement.textContent = 'Error accessing webcam. Ensure permissions are granted.';
        active = false;
        startBtn.disabled = false;
        stopBtn.disabled = true;
    }
    ...
    if (fileType === 'image') {
        ...
        imageElement.onload = async () => {
            ...
            try {
                ...
                statusElement.textContent = 'Error processing image. Check console for details.';
            }
            processUpload.disabled = false;
        ...
        videoElement.onloadedmetadata = async () => {
            console.log('Video loaded, processing...');
            canvasElement.width = videoElement.videoWidth;
            canvasElement.height = videoElement.videoHeight;
            inputImage = new OffscreenCanvas(videoElement.videoWidth, videoElement.videoHeight);
            inputCtx = inputImage.getContext('2d');
            try {
                ...
                statusElement.textContent = 'Error processing video. Check console for details.';
                active = false;
            }
            processUpload.disabled = false;
        };
...
function setupEventListeners() {
    ...
    stopBtn.addEventListener('click', () => {
        console.log('Stop button event triggered');
        stopProcessing();
    });
    modelSelect.addEventListener('change', (e) => {
        console.log('Model select changed to:', e.target.value);
    ...
    multiTrack.addEventListener('change', () => {
        console.log('Multi-person toggle changed to:', multiTrack.checked);
        if (active) {
            stopProcessing();
            if (sourceType === 'webcam') {
                startTracking();
    ...
    confSlider.addEventListener('input', (e) => {
        console.log('Confidence threshold changed to:', e.target.value);
        confLabel.textContent = `Min Confidence: ${e.target.value}`;
    });
    processUpload.addEventListener('click', () => {
        console.log('Process Upload button event triggered');
        processUploadedFile();
    });
    document.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'f') {
            showFace.checked = !showFace.checked;
            console.log('Show Face toggled to:', showFace.checked);
        }
        if (e.key === 'h') {
            showHands.checked = !showHands.checked;
            console.log('Show Hands toggled to:', showHands.checked);
        }
        if (e.key === 'p') {
            showPose.checked = !showPose.checked;
            console.log('Show Pose toggled to:', showPose.checked);
        }
        if (e.key === 'm') {
            showMeshes.checked = !showMeshes.checked;
            console.log('Show Meshes toggled to:', showMeshes.checked);
        }
    });
}
